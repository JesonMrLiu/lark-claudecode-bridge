...
// 触发词用于需要改写参数/附加指令的场景。rewrite 也可指向 bridge 本地命令
// （如飞书菜单场景 match=新会话, rewrite=/new）——改写结果由 wiring 回灌 handleCommand，
// 本地命令本地执行，绝不透传给 Claude Code（headless 侧没有这些命令）。
...
/**
 * 触发词的 rewrite 是否指向 bridge 本地命令（如 match=新会话, rewrite=/new）。
 * 两段式触发（ask_detail）据此防呆：本地命令无参数可补，追问只会挡住命令执行。
 */
export function isLocalCommandRewrite(rewrite: string): boolean {
  const firstToken = rewrite.trim().split(/\s+/)[0] ?? '';
  return firstToken.startsWith('/') && RESERVED_COMMANDS.has(firstToken.slice(1).toLowerCase());
}
...
/**
 * 命中触发词则返回命中的规则与改写后的 prompt，未命中返回 null（走原链路）。
 * rewriteByTrigger 的规则判定本体——需要知道命中了哪条规则（如 askDetail 两段式）时用本函数。
 *
 * 匹配语义：
 ...
 */
export function matchTrigger(text: string, rules: TriggerRule[] | undefined): { rule: TriggerRule; rewritten: string } | null {
  const t = text.trim();
