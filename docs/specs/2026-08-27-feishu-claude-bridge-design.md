...
**过程可见性（进度卡片全生命周期）**——确保用户任何时候都知道「机器人在干活，没卡死」：

1. **即时首响**：收到消息 1 秒内回复任务卡片（初始态「🚀 已接收，排队中/启动中」），绝不让消息无反馈
2. **流式刷新**：处理中同一张卡片实时 PATCH 更新（文本增量打字机效果 + 当前工具调用如 `🔧 Bash: npm test`），节流合并（字符积攒阈值 + 时间间隔）避免触发飞书限流；不逐条刷屏
3. **等待确认标记**：确认卡片发出时，进度卡片同步标记「⏸ 等待用户确认」
4. **长任务心跳**：工具运行期间无新事件时，卡片显示运行计时（「⏱ Bash 已运行 3 分 12 秒」定时刷新）——静默 ≠ 卡死
5. **收尾**：完成时进度卡片更新为任务摘要，随后新发结果卡片
